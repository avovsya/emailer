...
function addAttachment(letterId, attachment, cb) {
  async.waterfall([
    db.saveFile.bind(db, attachment),
    db.addLetterAttachmentId.bind(db, letterId)
  ], function (err, modifiedLettersCount) {
    if (err) return cb(err);
    if (modifiedLettersCount === 0) return cb(new Error('Not Found'));
    return cb();
  });
}
